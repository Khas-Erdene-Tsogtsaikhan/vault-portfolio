  ...
  return (
    <AppShell>
      <section className="mb-8">
        ...
        <p className="mt-4 max-w-2xl text-vault-muted">Search live comps, watch demand signals, and see price events for what you own or want. No marketplace noise. Just intelligence.</p>
      </section>
        ...
        <div className="vault-table">
          <div className="flex items-center justify-between border-b border-vault-border px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-vault-muted">Price events for owned and watched positions.</p>
            </div>
          ...
          {watchlist.map((watch) => {
            const delta = watch.currentPrice - watch.value24hAgo;
            return (
              <div key={watch.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 border-b border-vault-border bg-vault-gold/5 px-5 py-4 last:border-b-0">
                <span className="flex items-center gap-3">
                  <span className="relative h-9 w-9 overflow-hidden rounded border border-vault-border">
                    {watch.imageUrl ? <Image src={watch.imageUrl} alt="" fill sizes="36px" className="object-cover" /> : <Eye size={16} className="m-2 text-vault-gold" />}
                  </span>
                  <span>
                    <span className="block text-[13px] font-medium text-vault-text">{watch.name}</span>
                    <span className="block text-[11px] text-vault-faint">Watched · {watch.watchers} watchers</span>
                  </span>
                </span>
                <span className="data text-sm text-vault-text">{currency.format(watch.currentPrice)}</span>
                <span className={`data text-right text-sm ${delta >= 0 ? "text-vault-green" : "text-vault-red"}`}>{delta >= 0 ? "+" : ""}{currency.format(delta)}</span>
              </div>
            );
          })}
        </div>
        ...
        <aside className="space-y-6">
          <section className="vault-panel rounded-lg p-5">
            <div className="flex items-center justify-between">
              <p className="section-label">Demand Signals</p>
              <a href="/watchlist" className="text-xs text-vault-gold">Watchlist</a>
            </div>
            <div className="mt-5 space-y-3">
              {watchlist.slice(0, 4).map((watch) => (
                <div key={watch.id} className="flex items-center justify-between gap-3 rounded-md border border-vault-border bg-vault-surface p-3">
                  <div>
                    <p className="text-sm font-medium text-vault-text">{watch.name}</p>
                    <p className="mt-1 text-xs text-vault-muted">{watch.watchers} collectors watching</p>
                  </div>
            ...
            <div className="mt-5 space-y-3">
              {marketActivity.map((activity) => (
                <article key={activity.id} className="rounded-md border border-vault-border bg-vault-surface p-4">
                  ...
                  <p className="mt-3 flex items-center gap-2 text-[11px] text-vault-faint"><Bell size={13} /> Notification event logged for Phase 2.</p>
                </article>
              ))}
            </div>
          </section>
