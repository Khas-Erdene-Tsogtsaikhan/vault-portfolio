...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      const valuation = calculateMarketValue(mockSoldListings(query, category));
      const results = valuationToResults(query, category, valuation).slice(0, 5);
      return NextResponse.json({ results, valuation: publicValuation(valuation), source: "mock", note: `eBay Finding API returned ${response.status}; using fallback.` });
    }
    ...
    const valuation = calculateMarketValue(mockSoldListings(query, category));
    const results = valuationToResults(query, category, valuation).slice(0, 5);
    return NextResponse.json({ results, valuation: publicValuation(valuation), source: "mock", note: "eBay timeout or network error; keeping fallback data." });
  }
}
